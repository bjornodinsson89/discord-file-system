...
class RafflesRepository(RepositoryBase):
    async def create_raffle(
        ...
        prize: str,
        ticket_payment_type: str,  # Can now be "free"
        ticket_price: int,
        ...
        async with self.pool.acquire() as conn:
            ...
            row = await conn.fetchrow(
                ...
                prize,
                ticket_payment_type,
                0 if ticket_payment_type == "free" else ticket_price,  # Force price to 0 if free
                tickets_available,
                max_tickets_per_user,
                end_time,
                end_trigger,
                hours_after_sold_out,
                ticket_payment_type == "free"  # is_free boolean
            )
