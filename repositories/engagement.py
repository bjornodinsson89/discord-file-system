...
class EngagementRepository(RepositoryBase):
    ...
    async def update_guild_settings(self, guild_id: int, changes: dict[str, Any]) -> dict:
        if not changes:
        ...
        for key, value in changes.items():
            ...
            idx += 1
        sql = (
            "UPDATE engagement_guild_settings "
        ...
        async with self.acquire() as conn:
            ...
            _ = row
            updated = await conn.fetchrow(sql, *values)
            return dict(updated)
    ...
    async def leaderboard_xp(self, guild_id: int, limit: int = 10) -> list[dict]:
        async with self.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM engagement_profiles WHERE guild_id=$1 ORDER BY xp_total DESC, user_id ASC LIMIT $2",
                guild_id,
    ...
    async def leaderboard_levels(self, guild_id: int, limit: int = 10) -> list[dict]:
        async with self.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM engagement_profiles WHERE guild_id=$1 ORDER BY level DESC, xp_total DESC, user_id ASC LIMIT $2",
                guild_id,
                limit,
            )
            return [dict(r) for r in rows]

    async def leaderboard_tokens(self, guild_id: int, limit: int = 10) -> list[dict]:
        async with self.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM engagement_profiles WHERE guild_id=$1 ORDER BY prize_token_lifetime_earned DESC, user_id ASC LIMIT $2",
                guild_id,
                limit,
            )
            return [dict(r) for r in rows]

    async def leaderboard_jumps(self, guild_id: int, limit: int = 10) -> list[dict]:
        async with self.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM engagement_profiles WHERE guild_id=$1 ORDER BY jump_99k_completed_count DESC, user_id ASC LIMIT $2",
                guild_id,
                limit,
            )
            return [dict(r) for r in rows]

    async def leaderboard_raffles(self, guild_id: int, limit: int = 10) -> list[dict]:
        async with self.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM engagement_profiles WHERE guild_id=$1 ORDER BY paid_raffle_tickets_count DESC, user_id ASC LIMIT $2",
                guild_id,
                limit,
            )
            return [dict(r) for r in rows]
