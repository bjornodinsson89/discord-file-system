...
class RafflePaymentService:
    ...
    async def verify_raffle_payment(self, entry_id: int, manual: bool) -> tuple[bool, Optional[int], Optional[str]]:
        ...
        if buyer_key.get("torn_user_id") and int(buyer_key["torn_user_id"]) != int(buyer_torn_id or 0):
        ...
        try:
            if item_type == "xanax":
                direct_match = await torn_api.verify_xanax_payment(api_key, int(creator_torn_id), expected_qty, since_ts)
            elif item_type == "erotic_dvd":
                direct_match = await torn_api.verify_dvd_payment(api_key, int(creator_torn_id), expected_qty, since_ts)
        except TornAPIError:
            return False, None, "Verification temporarily unavailable. Try again."

        if direct_match and self._is_in_window(direct_match, since_ts, until_ts):
            match = direct_match
        else:
    ...
    @staticmethod
    def _is_in_window(entry: dict[str, Any], since_ts: int, until_ts: Optional[int]) -> bool:
        ts = RafflePaymentService._extract_timestamp(entry)
        if ts is None:
            return False
        if ts < since_ts:
            return False
