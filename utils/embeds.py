...
def create_jump_session_embed(
    session: Dict, signups: List[Dict], readiness: List[Dict] = None
) -> discord.Embed:
    status = session["status"]
    status_emoji = {
        "open": config.EMOJI_UNLOCK,
        "locked": config.EMOJI_LOCK,
        "completed": config.EMOJI_CHECK,
        "cancelled": config.EMOJI_CROSS,
    }.get(status, "")
    ...
    embed.add_field(
        name=f"{config.EMOJI_PILL} Xanax", value=f"`{session['xanax_count']}`", inline=True
    )
    ...
    if signups:
        ...
        embed.add_field(
            name=f"Participants ({len(signups)}/{session['max_spots']})",
            value="\n".join(parts),
            inline=False,
        )
...
def create_insurance_policy_embed(policy: Dict) -> discord.Embed:
    embed = create_base_embed(
        f"{config.EMOJI_SHIELD} {policy['name']}", policy.get("description"), config.COLOR_INSURANCE
    )
    embed.add_field(
        name="Provider",
        value=f"<@{policy['provider_discord_id']}> ({policy['company_name']})",
        inline=True,
    )
    embed.add_field(
        name="Coverage Types",
        value=", ".join(policy.get("covered_jump_types", ["99k"])),
        inline=True,
    )
    embed.add_field(
        name=f"{config.EMOJI_PILL} Max Coverage",
        value=f"{policy['max_coverage_xanax']} Xanax",
        inline=True,
    )
    embed.add_field(
        name=f"{config.EMOJI_MONEY} Premium",
        value=f"${policy['premium_per_xanax']:,}/Xanax",
        inline=True,
    )
    embed.add_field(
        name=f"{config.EMOJI_TROPHY} Payout",
        value=f"${policy['payout_per_xanax']:,}/Xanax",
        inline=True,
    )
    embed.add_field(
        name=f"{config.EMOJI_CLOCK} Duration",
        value=f"{policy['duration_hours']} hours",
        inline=True,
    )
    return embed
...
def create_coverage_embed(coverage: Dict) -> discord.Embed:
    embed = create_base_embed(
        f"{config.EMOJI_SHIELD} Your Insurance Coverage", color=config.COLOR_INSURANCE
    )
    embed.add_field(
        name="Policy", value=f"{coverage['policy_name']} ({coverage['company_name']})", inline=False
    )
    ...
    embed.add_field(
        name="Expires", value=f"<t:{int(coverage['expires_at'].timestamp())}:R>", inline=True
    )
    return embed
...
def create_claim_embed(claim: Dict) -> discord.Embed:
    status_colors = {
        ...
        "approved": config.COLOR_INFO,
        "paid": config.COLOR_SUCCESS,
        "denied": config.COLOR_ERROR,
    }
    embed = create_base_embed(
        f"{config.EMOJI_SHIELD} Claim #{claim['claim_id']}",
        color=status_colors.get(claim["status"], config.COLOR_SECONDARY),
    )
    embed.add_field(
        name="Policy", value=f"{claim['policy_name']} ({claim['company_name']})", inline=False
    )
    embed.add_field(
        name="User",
        value=f"<@{claim['user_discord_id']}> (Torn: {claim['user_torn_id']})",
        inline=True,
    )
    payout_line = payout_items_to_human(claim.get("payout_items") or [])
    embed.add_field(
        name="Payout",
        value=payout_line if payout_line != "Not set" else f"${claim['payout_amount']:,}",
        inline=True,
    )
...
def create_raffle_embed(raffle: Dict, entries: List[Dict]) -> discord.Embed:
    status_emoji = {
        "active": config.EMOJI_TICKET,
        "drawing": config.EMOJI_DICE,
        "completed": config.EMOJI_TROPHY,
        "cancelled": config.EMOJI_CROSS,
    }.get(raffle["status"], "")
    ...
    embed.add_field(
        name=f"{config.EMOJI_USER} Host", value=f"<@{raffle['creator_discord_id']}>", inline=True
    )
    ...
    reserved_entries = [
        e
        for e in entries
        if not e.get("payment_verified")
        and e.get("reserved_until")
        and e["reserved_until"] > now_utc
    ]
    ...
    total_tickets = (
        raffle.get("tickets_sold") if raffle.get("tickets_sold") is not None else paid_tickets
    )

    embed.add_field(
        name=f"{config.EMOJI_TICKET} Tickets Sold",
        value=f"{total_tickets}/{raffle['tickets_available']}",
        inline=True,
    )
    ...
    embed.add_field(
        name="Max Per User",
        value=(
            f"{raffle['max_tickets_per_user']} tickets"
            if raffle.get("max_tickets_per_user")
            else "Unlimited"
        ),
        inline=True,
    )
    ...
    if raffle["status"] == "completed" and raffle.get("winner_discord_id"):
        embed.add_field(
            name=f"{config.EMOJI_TROPHY} Winner",
            value=f"<@{raffle['winner_discord_id']}> (Ticket #{raffle.get('winner_ticket_number', '?')})",
            inline=False,
        )
...
def create_host_reputation_embed(discord_id: int, rep: Dict) -> discord.Embed:
    ...
    embed.add_field(
        name=f"{config.EMOJI_CHECK} Positive", value=str(rep["positive_ratings"]), inline=True
    )
    embed.add_field(
        name=f"{config.EMOJI_CROSS} Negative", value=str(rep["negative_ratings"]), inline=True
    )
...
def create_statistics_embed(stats: Dict, title: str = "Statistics") -> discord.Embed:
    ...
    if "sessions" in stats:
        s = stats["sessions"]
        embed.add_field(
            name=f"{config.EMOJI_JUMP} Jump Sessions",
    ...
    if "raffles" in stats:
        r = stats["raffles"]
        embed.add_field(
            name=f"{config.EMOJI_TICKET} Raffles",
    ...
    if "registered_users" in stats:
        embed.add_field(
            name=f"{config.EMOJI_USER} Registered Users",
            value=str(stats["registered_users"]),
            inline=True,
        )
    ...
    if "insurance_claims" in stats:
        ...
        embed.add_field(
            name="Insurance Claims", value=f"Total: {c['total']}\nPaid: {c['paid']}", inline=True
        )
...
def create_api_key_guide_embed() -> discord.Embed:
    ...
    embed.add_field(
        name=f"{config.EMOJI_LOCK} Security",
...
def create_blacklist_embed(blacklist: List[Dict]) -> discord.Embed:
    ...
    for entry in blacklist[:20]:
        exp = (
            f" (until <t:{int(entry['expires_at'].timestamp())}:R>)"
...
def create_session_announcement_embed(session: Dict, guild) -> discord.Embed:
    ...
    embed.add_field(name=f"{config.EMOJI_MONEY} Payment Required", value=payment, inline=True)
    ...
    embed.add_field(name="Status", value=f"{config.EMOJI_UNLOCK} Open", inline=True)
...
def create_raffle_announcement_embed(raffle: Dict, guild) -> discord.Embed:
    ...
    embed = create_base_embed(
        f"{config.EMOJI_TICKET} New Raffle!", raffle["prize"], config.COLOR_RAFFLE
    )
    ...
    embed.add_field(name=f"{config.EMOJI_MONEY} Ticket Price", value=price, inline=True)
    ...
    if raffle.get("max_tickets_per_user", 0) > 0:
        embed.add_field(
            name="Max Per User", value=f"{raffle['max_tickets_per_user']} tickets", inline=True
        )
    else:
        embed.add_field(name="Max Per User", value="Unlimited", inline=True)
...
def create_policy_announcement_embed(policy: Dict, guild) -> discord.Embed:
    ...
    embed.add_field(name="Policy Name", value=policy["name"], inline=True)

    if policy.get("company_name"):
        embed.add_field(name="Provider", value=policy["company_name"], inline=True)
    ...
    embed.add_field(name=f"{config.EMOJI_MONEY} Cost", value=cost, inline=True)
    ...
    embed.add_field(
        name="Coverage Type", value=coverage_labels.get(coverage_type, coverage_type), inline=True
    )
    ...
    if payout_text != "Not set":
        embed.add_field(
            name=f"{config.EMOJI_TROPHY} Payout", value=f"Payout: {payout_text}", inline=False
        )
    elif policy.get("payout_description"):
        embed.add_field(
            name=f"{config.EMOJI_TROPHY} Payout", value=policy["payout_description"], inline=False
        )

    embed.set_footer(
        text=f"Policy #{policy['policy_id']} • Click button below to purchase coverage"
    )
    return embed
...
def create_claim_notification_embed(claim: Dict, coverage: Dict) -> discord.Embed:
    ...
    embed.add_field(
        name="Claim Type", value=claim["claim_type"].replace("_", " ").title(), inline=True
    )

    embed.add_field(
        name="Payout", value=payout_items_to_human(claim.get("payout_items") or []), inline=True
    )
