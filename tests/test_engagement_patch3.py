...
class _FakeRepo:
    def __init__(self):
        ...
        self.level_rewards = [
            ...
            {"level_required": 50, "role_id": 101, "role_name": "City Kingpin", "role_color": "FD79A8"},
        ]
        self.prize_rewards = [
            {"milestone_type": "lifetime_tokens_earned", "milestone_value": 5, "role_id": 201, "role_name": "Supporter", "role_color": "00CEC9"},
            {"milestone_type": "jump_completions", "milestone_value": 3, "role_id": 202, "role_name": "Jump Starter", "role_color": "74B9FF"},
        ]
    ...
    async def set_prize_reward_role_id(self, _guild_id: int, milestone_type: str, milestone_value: int, role_id: int):
        for row in self.prize_rewards:
            if row["milestone_type"] == milestone_type and row["milestone_value"] == milestone_value:
                row["role_id"] = role_id
...
def test_role_sync_keeps_highest_level_and_stacks_activity_roles():
    async def _run():
        ...
        repo.level_rewards = [
            ...
            {"level_required": 50, "role_id": 102, "role_name": "City Kingpin", "role_color": "FD79A8"},
        ]
        repo.prize_rewards = [
            {"milestone_type": "lifetime_tokens_earned", "milestone_value": 5, "role_id": 201, "role_name": "Supporter", "role_color": "00CEC9"},
            {"milestone_type": "jump_completions", "milestone_value": 3, "role_id": 202, "role_name": "Jump Starter", "role_color": "74B9FF"},
        ...
        guild = _FakeGuild(1, [_FakeRole(100), _FakeRole(101), _FakeRole(102), _FakeRole(201), _FakeRole(202)])
        member = _FakeMember(9, roles=[guild.get_role(100), guild.get_role(101)])

        result = await service.sync_member_roles(guild, member, {"level": 80, "prize_token_lifetime_earned": 10, "jump_99k_completed_count": 4})
...
def test_patch3_sources_include_setup_role_actions_and_migration_metadata_columns():
    ...
    migration_src = Path("migrations/2026_03_20_engagement_reward_role_metadata.sql").read_text(encoding="utf-8")
...
def test_locked_seed_definitions_have_expected_counts_and_thresholds():
    ...
    assert [level for level, _, _ in LEVEL_ROLE_DEFINITIONS] == [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 43, 46, 49, 50]
