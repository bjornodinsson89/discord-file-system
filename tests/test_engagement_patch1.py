...
def test_voice_xp_dedupe_and_totals():
    import asyncio

    async def _run():
        repo = _FakeRepo()
        service = EngagementService(repo, _FakePrizeTokenService())
        ok = await service.voice_xp_if_eligible(
            guild_id=1,
            user_id=2,
            channel_id=99,
            minute_bucket=123,
        ...
        assert ok is True
        dup = await service.voice_xp_if_eligible(
            guild_id=1,
            user_id=2,
            channel_id=99,
            minute_bucket=123,
        ...
        assert dup is False
        profile = repo.profiles[(1, 2)]
        assert profile["xp_total"] == 5
        assert profile["voice_xp_total"] == 5

    asyncio.run(_run())


def test_patch2_commands_and_setup_views_present():
    engagement_src = open("cogs/engagement.py", encoding="utf-8").read()
    setup_src = open("setup_panel.py", encoding="utf-8").read()

    assert '@profile.command(name="rewards"' in engagement_src
    assert '@leaderboard.command(name="xp"' in engagement_src
    assert '@leaderboard.command(name="levels"' in engagement_src
    assert '@leaderboard.command(name="tokens"' in engagement_src
    assert '@leaderboard.command(name="jumps"' in engagement_src
    assert '@leaderboard.command(name="raffles"' in engagement_src
    assert '@tokens.command(name="history"' in engagement_src
    assert '@engagement.command(name="config"' in engagement_src
    assert "voice_xp_worker" in engagement_src
    assert "reached Level {int(level)} and earned 1 Prize Token." in engagement_src

    assert "class EngagementCoreView" in setup_src
    assert "class EngagementChatVoiceReactionView" in setup_src
    assert "class EngagementEventXpView" in setup_src
    assert 'label="Engagement"' in setup_src


def test_leaderboard_repository_sorting_sql_present():
    src = open("repositories/engagement.py", encoding="utf-8").read()
    assert "ORDER BY level DESC, xp_total DESC" in src
    assert "ORDER BY prize_token_lifetime_earned DESC" in src
    assert "ORDER BY jump_99k_completed_count DESC" in src
    assert "ORDER BY paid_raffle_tickets_count DESC" in src
